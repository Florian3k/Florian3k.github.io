function Firework() {
  ...
  this.expolde = function() {
    ...
    for (var i = -10; i <= 10; i += 2 ) {
      this.particles.push(new Particle(this.firework.pos.x,this.firework.pos.y, this.hue, createVector(abs(i)-5+shift, i) ));
    }
    shift += 17;
    // O
    ...
    for (var alpha = 0; alpha < TWO_PI; alpha += PI/10) {
      ...
      var x = sin(alpha) * r;
      var y = cos(alpha) * r;
      this.particles.push(new Particle(this.firework.pos.x,this.firework.pos.y, this.hue, createVector(x+shift,y) ));
    }
    shift += 19;
    // N
    for (var i = -10; i <= 10; i += 2.5 ) {
    ...
    for (var i = -5; i <= 0; i += 2.5) {
      this.particles.push(new Particle(this.firework.pos.x,this.firework.pos.y, this.hue, createVector(i+shift, -10) ));
      this.particles.push(new Particle(this.firework.pos.x,this.firework.pos.y, this.hue, createVector(i+shift, 0) ));
    }

    for (var alpha = 0; alpha < PI; alpha += PI/6) {
      var r = 5;
      var x = sin(alpha) * r;
      var y = cos(alpha) * r;
      this.particles.push(new Particle(this.firework.pos.x,this.firework.pos.y, this.hue, createVector(x+shift,y-5) ));
    }
    shift += 15;
    // A
    for (var i = -10; i <= 10; i += 2.5 ) {
    ...
    for (var alpha = 0; alpha < PI; alpha += PI/10) {
      var r = 10;
      var x = sin(alpha) * r;
      var y = cos(alpha) * r;
